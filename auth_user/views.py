...
from .serializers import OtpGenerationSerializer, OtpVerificationSerializer, UserSerializer
from random_otp.generator import generate_numeric_otp
from django.utils import timezone
from lib.otp import send_otp_sms
from drf_yasg.utils import swagger_auto_schema
...
@swagger_auto_schema(
    method='post',
    request_body=OtpGenerationSerializer,
    responses={
        200: openapi.Response(
            description='OTP generated and sent successfully',
            examples={
...
def generate_otp(request):
    ...
    if serializer.is_valid():
        ...
        OtpHistory.objects.update_or_create(
            phone_number=phone_number,
            defaults={
                'otp': otp,
...
def verify_otp(request):
    ...
    if serializer.is_valid():
        ...
        if not otp_history or otp_history.is_expired():
            return Response({'error': 'Invalid or expired OTP'}, status=status.HTTP_400_BAD_REQUEST)
...
def register(request):
    ...
    # Scenario 2: If phone number, username, and OTP are provided, complete the registration
    if username and otp:
        # Verify if the OTP is correct
        otp_history = OtpHistory.objects.filter(phone_number=phone_number, otp=otp).first()
    ...
    return Response({'error': 'Invalid request. Please provide phone_number for OTP generation, or phone_number, username, and OTP for registration.'}, status=status.HTTP_400_BAD_REQUEST)